...
class PLE(BaseModel):
    ...
    def __init__(self, dnn_feature_columns, shared_expert_num=1, specific_expert_num=1, num_levels=2,
        ...
        dnn_linear_in_feature = 2 + expert_dnn_hidden_units[-1]  # linear + net + bottom
        if backbone == 'deepfm':   
            self.fm = FM()
        ...
        self.tower_dnn_final_layer = nn.ModuleList([nn.Linear(
            tower_dnn_hidden_units[-1] if len(tower_dnn_hidden_units) > 0 else dnn_linear_in_feature, 1,
            bias=False)
    ...
    # a single cgc Layer
    def cgc_net(self, inputs, level_num, temperature=1):
        ...
        for i in range(self.num_tasks):
            ...
            gate_mul_expert = torch.matmul((gate_dnn_out * temperature).softmax(1).unsqueeze(1), cur_experts_outputs)  # (bs, 1, dim)
    ...
    def forward(self, X):
        ...
        for i in range(self.num_tasks):
            if len(self.tower_dnn_hidden_units) > 0:
                tower_dnn_out = self.tower_dnn[i](torch.cat((other_output, ple_outputs[i]), dim=-1))
                tower_dnn_logit = self.tower_dnn_final_layer[i](tower_dnn_out)
            else:
                tower_dnn_logit = self.tower_dnn_final_layer[i](torch.cat((other_output, ple_outputs[i]), dim=-1))
