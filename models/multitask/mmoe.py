...
class MMOE(BaseModel):
    ...
    def __init__(self, dnn_feature_columns, num_experts=3, expert_dnn_hidden_units=(256, 128),
                 ...
                 init_std=0.0001, dnn_dropout=0, dnn_activation='relu', dnn_use_bn=False,
    ...
    def forward(self, X):
        ...
        for i in range(self.num_tasks):
            if len(self.tower_dnn_hidden_units) > 0:
                tower_dnn_out = self.tower_dnn[i](torch.cat((other_output, mmoe_outs[i]), dim=-1))
                tower_dnn_logit = self.tower_dnn_final_layer[i](tower_dnn_out)
            else:
                tower_dnn_logit = self.tower_dnn_final_layer[i](torch.cat((other_output, mmoe_outs[i]), dim=-1))
