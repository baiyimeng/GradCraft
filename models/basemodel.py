...
from models.inputs import build_input_features, SparseFeat, DenseFeat, VarLenSparseFeat, get_varlen_pooling_list, create_embedding_matrix, varlen_embedding_lookup
...
from solvers.min_norm_solvers_numpy import MinNormSolverNumpy
from solvers.grad_norm import GradNorm
from solvers.pc_grad import PCGrad
from solvers.pc_grad_magnitude_positive import PCGradMagnitudePositive
from solvers.grad_vac import GradVac
from solvers.imtl import IMTL
from solvers.dbmtl import DBMTL
from solvers.global_pc_grad import GlobalPCGrad
from solvers.global_pc_grad_magnitude_positive import GlobalPCGradMagnitudePositive
from solvers.ca_grad import CAGrad
...
class BaseModel(nn.Module):
    def __init__(self, linear_feature_columns, dnn_feature_columns, l2_reg_linear=1e-5, l2_reg_embedding=1e-5,
                 init_std=0.0001, task='binary', device='cpu', gpus=None):
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
        ...
        if test_data:
            ...
            if len(test_data) == 2:
                ...
                test_x, test_y, test_sample_weight = test_data  # pylint: disable=unpacking-non-sequence
            else:
                raise ValueError(
        ...
        train_loader = DataLoader(
            dataset=train_tensor_data, shuffle=shuffle, batch_size=batch_size, pin_memory=True, num_workers=4)
        ...
        print("Train on {0} samples, validate on {1} samples, test on {2} samples, {3} steps per epoch".format(
            len(train_tensor_data), len(val_y), len(test_y), steps_per_epoch))

        
        # The following methods include additional learnable parameters
        if method in ['uncertainty', 'imtl', 'imtl-l']:
            weights = torch.ones(self.num_tasks)
            weights = (weights / sum(weights)).to(self.device)
            weights = nn.Parameter(weights)
            optim.add_param_group({'params': weights})


        # Set a customized optimizer for the specific method ['gradnorm', 'pcgrad', 'pcgrad+', 'gradvac', 'imtl', 'imtl-g', 'dbmtl', 'gpcgrad', 'gradcraft', 'cagrad']
        # Note: ['single', 'average', 'uncertainty', 'imtl-l', 'dwa', 'mgda'] do not require a custom optimizer as they are straightforward to implement
        def customized_optimizer(method, optimizer, num_tasks, hyper=None, device=None):
            if method == 'gradnorm':
                return GradNorm(optimizer, num_tasks, lr_weights=optimizer.defaults['lr'], alpha=0.5, device=device)
            
            elif method == 'pcgrad':
                return PCGrad(optimizer)
            
            elif method == 'pcgrad+':
                return PCGradMagnitudePositive(optimizer, num_tasks, b=0, r=hyper['r'], e=hyper['e'], device=device)
            
            elif method == 'gradvac':
                return GradVac(optimizer, num_tasks, beta=1e-2, device=device)
            
            elif method in ['imtl', 'imtl-g']:
                return IMTL(optimizer)
            
            elif method == 'dbmtl':
                return DBMTL(optimizer)
            
            elif method == 'gpcgrad':
                return GlobalPCGrad(optimizer)
            
            elif method == 'gradcraft':
                return GlobalPCGradMagnitudePositive(optimizer, num_tasks, b=0, r=hyper['r'], e=hyper['e'], device=device)
            
            elif method == 'cagrad':
                return CAGrad(optimizer, c=hyper['c'], rescale=0)
        ...
        for epoch in range(initial_epoch, epochs):
                    ...
                    for step, (x_train, y_train) in t:
                        ...
                        if isinstance(loss_func, list):
                            ...
                            if method == 'single':
                                ...
                                i = self.task_names.index(label)
                                loss = loss_list[i]
                            
                            elif method == 'average':
                                y_pred = model(x).squeeze()
                                loss_list = [loss_func[i](y_pred[:, i], y[:, i], reduction='mean') for i in range(self.num_tasks)]
                                loss = sum(loss_list) / self.num_tasks

                            elif method == 'uncertainty':
                                y_pred = model(x).squeeze()
                                loss_list = [loss_func[i](y_pred[:, i], y[:, i], reduction='mean') for i in range(self.num_tasks)]
                                loss = torch.dot(weights, torch.stack(loss_list)) + torch.sum(torch.log(1 + 0.5 / weights))

                            elif method == 'imtl-l':
                                y_pred = model(x).squeeze()
                                loss_list = [loss_func[i](y_pred[:, i], y[:, i], reduction='mean') for i in range(self.num_tasks)]
                                scaled_loss_list = [torch.exp(weights)[i] * l - weights[i] for i, l in enumerate(loss_list)]
                                loss = sum(scaled_loss_list) / self.num_tasks
                                ...
                                if step == 0:
                                    loss = sum(loss_list) / self.num_tasks
                                    loss_t_minus_2 = torch.stack(loss_list).detach()
                                elif step == 1:
                                    loss = sum(loss_list) / self.num_tasks
                                    loss_t_minus_1 = torch.stack(loss_list).detach()
                                else:
                                    weights = (loss_t_minus_1 / loss_t_minus_2) / 2
                                    weights = F.softmax(weights, dim=0)
                                    loss = torch.dot(weights, torch.stack(loss_list))
                                    loss_t_minus_2 = loss_t_minus_1
                                    loss_t_minus_1 = torch.stack(loss_list).detach()

                            elif method == 'mgda':
                                y_pred = model(x).squeeze()
                                loss_list = [loss_func[i](y_pred[:, i], y[:, i], reduction='mean') for i in range(self.num_tasks)]
                                index = list(range(0, self.num_tasks))
                                grads = {}
                                for i in index:
                                    optim.zero_grad()
                                ...
                                grads_list = [torch.cat(grads[i]).data for i in index]

                                grads_input = [g.data.cpu().numpy() for g in grads_list]
                                weights, _ = MinNormSolverNumpy.find_min_norm_element(grads_input)
                                weights = weights.astype(np.float32)
                                weights = torch.tensor(weights).to(self.device)

                                loss = torch.dot(weights, torch.stack(loss_list)[index])
                                ...
                                optim.imtl_backward(scaled_loss_list)
                                optim.step()
                                ...
                                loss_epoch += torch.stack(loss_list).data.cpu().numpy() * len(y)
                                total_loss_epoch += (sum(loss_list) / self.num_tasks + reg_loss).item() * len(y)
                                continue
                        ...
                        loss_epoch += torch.stack(loss_list).data.cpu().numpy() * len(y)
                        total_loss_epoch += total_loss.item() * len(y)
            ...
            if verbose > 0:
                ...
                eval_str += " - " + "total_loss" + ": {0: .4f}".format(epoch_logs["total_loss"])
                ...
                if do_validation:
                    if self.num_tasks == 1:
                        ...
                        for i in range(self.num_tasks):
                            for name in self.metrics:
                                eval_str += " - " + "val_" + name + "_" + self.task_names[i] + \
                                            ": {0: .4f}".format(epoch_logs["val_" + name + "_" + self.task_names[i]])
                ...
                if do_test:
                    if self.num_tasks == 1:
                        ...
                        for i in range(self.num_tasks):
                            for name in self.metrics:
                                eval_str += " - " + "test_" + name + "_" + self.task_names[i] + \
                                            ": {0: .4f}".format(epoch_logs["test_" + name + "_" + self.task_names[i]])
                print(eval_str)

            # ray.tune
            if self.use_tune:
                epoch_logs_eval = {name: value for name, value in epoch_logs.items() if name != 'total_loss'}
        ...
        if self.num_tasks == 1:
            ...
            for name, metric_fun in self.metrics.items():
                if name != 'gauc':
                    ...
                    eval_result[name] = metric_fun(y, pred_ans, np.squeeze(x[self.feature_index['user_id'][0]]))
        else:
            pred_ans = self.predict(x, batch_size)
            eval_result = {}
            for i in range(self.num_tasks):
                for name, metric_fun in self.metrics.items():
                    if name != 'gauc':
                        eval_result[name+'_'+self.task_names[i]] = metric_fun(y[:, i], pred_ans[:, i])
                    else:
                        eval_result[name+'_'+self.task_names[i]] = metric_fun(y[:, i], pred_ans[:, i], np.squeeze(x[self.feature_index['user_id'][0]]))
        return eval_result
    ...
    def predict(self, x, batch_size=256):
        ...
        test_loader = DataLoader(
            dataset=tensor_data, shuffle=False, batch_size=batch_size, pin_memory=True, num_workers=4)
    ...
    def _get_metrics(self, metrics, set_eps=False):
        ...
        if metrics:
            for metric in metrics:
                ...
                if metric == "gauc":
                    metrics_[metric] = self._roc_auc_score_group
                if metric == "mse":
