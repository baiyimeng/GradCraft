...
class ExperimentAnalysis:
    ...
    def get_best_trial(
        ...
        if isinstance(metric, str):
            ...
            metric_ = []
            for m in metric:
                metric_.append(self._validate_metric(m))
            metric = metric_
        ...
        for trial in self.trials:
            if isinstance(metric, str):
                ...
                if any(m not in trial.metric_analysis for m in metric):
                    continue

                if scope in ["last", "avg", "last-5-avg", "last-10-avg"]:
                    metric_score = 0
                    for m in metric:
                        metric_score += trial.metric_analysis[m][scope]
                    metric_score = metric_score / len(metric)
                else:
                    metric_score = 0
                    for m in metric:
                        metric_score += trial.metric_analysis[m][mode]
                    metric_score = metric_score / len(metric)
